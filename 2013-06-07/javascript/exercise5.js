...
var roadLength = 170;
var distanceConstant = 20;

function drawRoad(roadLength, distanceConstant) {
	var width = 2.2;
	var cube = CUBOID([roadLength,0.2,width]);
	var line = COLOR([1.5,1.5,1.5,1.5])(T([1,2])([1.1,0.05])(cube));
	var mainRoadCube = CUBOID([roadLength,width,width])
	var mainRoad0 = COLOR([0,0,0,1])(mainRoadCube);
	var mainRoad = STRUCT([mainRoad0, line]);
	var road = STRUCT([mainRoad]);
	var distance = (width*distanceConstant);

	if (Math.floor(roadLength/distance)>1) {
		var incNum = Math.floor(roadLength/distance);
		for(i=1; i<incNum; i++) {
			var lineCube = CUBOID([0.2,roadLength/2,width]);
 			var line = COLOR([1.5,1.5,1.5,1.5])(lineCube);
 			var incCube = CUBOID([width, roadLength/2, width]);
 			var inc = COLOR([0,0,0,1])(incCube);
			var road = STRUCT([road,T([0,1])([distance*i, -roadLength/4])(inc),T([0,1,2])([distance*i+1.1, -roadLength/4,0.05])(line)])
		}
	}
	return road;
}

var road = T([0,1,2])([5,3,altitudeHill-1.5])(S([0,1,2])([0.025,0.025,0.025])(T([0])([-7])(R([0,1])([PI/2,PI/2])(drawRoad(roadLength, distanceConstant)))));
DRAW(road);